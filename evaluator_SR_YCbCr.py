...
class Eval_thread():
    def __init__(self, epoch, model, loader, method, dataset, output_dir, cuda):
        self.model = model 
        model.eval() 
        self.epoch = epoch
        self.loader = loader
        ...
        self.logfile = os.path.join(output_dir, 'result_224*224_bs=8_'+ method +'_'+dataset+'.txt')
    ...
    def run(self):
        ...
        self.LOG(
            'Epoch{:<3d}: {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} S-measure.\n'
            .format(self.epoch, self.dataset, self.method, mae, max_f, s))
    ...
    def Eval_mae(self):
        ...
        with torch.no_grad():
            for image_lr_rgb, img_YCbCr_lr, img_YCbCr_sr, gt, name in tqdm(self.loader):  # img_lr, img_sr, label, name
                ###input RGB space and YCbCr space
                input1 = image_lr_rgb.cuda()
                input2 = img_YCbCr_lr.cuda()
                
                if self.cuda:
                    pred  = self.model(input1, input2) 
                    pred = pred.cuda()
                    gt = gt.cuda()
                else:
                    pred  = self.model(input1, input2)  
    ...
    def Eval_fmeasure(self):
        ...
        with torch.no_grad():
            for image_lr_rgb, img_YCbCr_lr, img_YCbCr_sr, gt, name in tqdm(self.loader):
                ###input RGB space and YCbCr space
                input1 = image_lr_rgb.cuda()
                input2 = img_YCbCr_lr.cuda()
                if self.cuda:
                    pred   = self.model(input1, input2)  # pred is the return of model.eval 
                    pred = pred.cuda()
                    gt = gt.cuda()
